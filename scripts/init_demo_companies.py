"""
Initialize demo tenant companies and accounts.
...
Usage:
    $env:DEMO_ACCOUNT_PASSWORD='<temporary-demo-password>'
    python scripts/init_demo_companies.py
    python scripts/init_demo_companies.py --reset-passwords
"""
...
DEMO_ACCOUNT_PASSWORD_ENV = 'DEMO_ACCOUNT_PASSWORD'
    ...
    *,
    ...
    if existing is None:
        if password_hash is None:
            raise RuntimeError('Demo account creation requires a demo password hash.')
        existing = Account(username=account.username, password_hash=password_hash)
        session.add(existing)
    elif reset_passwords:
        if password_hash is None:
            raise RuntimeError('Demo account reset requires a demo password hash.')
...
def _read_demo_account_password() -> str:
    value = os.environ.get(DEMO_ACCOUNT_PASSWORD_ENV, '')
    if not value:
        raise SystemExit(
            f'Set {DEMO_ACCOUNT_PASSWORD_ENV} before creating demo accounts or using --reset-passwords.'
        )
    return value


def initialize_demo_companies(reset_passwords: bool = False) -> None:
        ...
        for tenant in DEMO_TENANTS:
            ...
            for demo_account in tenant.accounts:
                ...
                if (account_exists is None or reset_passwords) and password_hash is None:
                    password_hash = hash_password(_read_demo_account_password())
                _upsert_account(
                    session,
    ...
    print('Initialized demo companies and demo accounts.')
...
def main() -> None:
    parser = argparse.ArgumentParser(description='Initialize demo tenant companies and demo accounts.')
    parser.add_argument(
        '--reset-passwords',
        action='store_true',
    ...
    initialize_demo_companies(reset_passwords=args.reset_passwords)
