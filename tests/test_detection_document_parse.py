...
def test_parse_detection_text_extracts_candidate_rows():
    text = '\n'.join(
        [
            '检测点 喷漆岗 苯 50000 μg/m3 采样时长 60 min',
            '空压机房 噪声 88 dB(A) 8 h',
            '炼钢平台 WBGT 31 ℃',
        ]
    )
    ...
    assert rows[0].indicator_name == '苯'
...
def test_preview_detection_document_from_docx(client: TestClient, admin_token: str, tmp_path: Path):
    ...
    assert body['rows'][0]['indicator_name'] == '苯'
...
def test_parse_detection_docx_tables_for_anonymized_sample_if_present():
    ...
    assert any(row.indicator_name == '其他粉尘' for row in rows)
    assert any(row.indicator_name.startswith('高温WBGT-II级-100%') for row in rows)
    assert any(row.is_below_detection_limit for row in rows)
    assert any(row.is_background for row in rows)

    raw_record_rows = parsed_by_file.get('GS-TR071-01粉尘分散度原始记录(1).docx')
