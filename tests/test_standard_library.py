...
def test_standard_manifest_import_and_search_without_file_upload(
                ...
                'chunks': [
                    {
                        'chunk_index': 0,
                        'clause': '4.1',
                        'text_chunk': '苯的职业接触限值依据：PC-TWA 为 6 mg/m3。',
                        'page_start': 12,
                        'page_end': 12,
                    }
                ],
    ...
    assert doc.source_path == 'minio://ehs-standard-library/raw/occupational_health/test-std-001.pdf'

    search = client.get(
        '/api/v1/standards/chunks/search',
        params={'q': '苯 PC-TWA', 'limit': 5},
    ...
    assert '苯的职业接触限值依据' in items[0]['text_chunk']
                    ...
                    'chunks': [
                        {
                            'chunk_index': 0,
                            'clause': '5.2',
                            'text_chunk': '苯的职业接触限值依据来自 TEST-RAG-001 第 5.2 条。',
                        }
                    ],
                }
            ]
